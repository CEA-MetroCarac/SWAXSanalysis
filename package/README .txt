...
If you cannot see the markdown version of this version you can either :
- go to this website : https://stackedit.io/, click start writing and copy and paste this txt file into the left part of the screen. 
Although, you wont be able to see the image
- Download a markdown editor like obsidian and open this package as a new project and then click on the README.md file
- Use pycharm and open this package and then the README.md file

It is recommended to open the .md version since you'll be able to see the images. if you really can't you can open said images in the other/image README file
...
![alt-text](./others/image%20README/firstPopUp.png "first popup")\
...
### Simple mode 
![alt-text](./others/image%20README/SimpleMode.png "Simple mode")

This is a preview of the GUI in simple mode, as you can see, you have :

1. A section to enter the **absolute path of the file you want to export** (in lavender), and a browse button if you want to enter 
it after searchin for its location in your files
2. A section to enter the **absolute path of the settings file you want to use to export your original file** (in lavender), 
...
4. **An "export file" button** (in green) that exports the .edf to a .h5 file in the same folder that contained your original file.
...
### Advanced mode
![alt-text](./others/image%20README/AdvancedMode.PNG "Simple mode with fillable parameters")
...
- **A "Create autofill settings** (in green) settings" that will open a new window, we'll talk about thta in the next section
- 3 columns :
  ...
  3. A combobox that lets you **chose a unit** from a list of allowed units. This is the unit of the value as it is in the 
...
![alt-text](./others/image%20README/AutoffilSettings.PNG "The first autofill settings window")

This first step displays all the metadata available in the header, that is the **name of the metadata (1), and it's value (2)**. 
...
Once all the desired metadata have been selected, **you can click next step (in green)**, **confirm** all the parameters you want to keep 
are on hte pop-up, and then it will show another step :

![alt-text](./others/image%20README/Autofillsettings2.PNG "The first autofill settings window")
...
the experimental data in the .edf file (in orange). I recommend to be as precise as possible, use only alphanumerical caracters
...
- The export doesn't work because a value could not be converted : you forgot, our put the wrong unit during the creation 
